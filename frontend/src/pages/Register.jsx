...
export default function Register() {
    ...
    const validateDateOfBirth = (date) => {
        if (!date || date.includes('undefined') || date.split('-').length < 3) return 'La fecha de nacimiento es requerida'
        const [year, month, day] = date.split('-')
        if (!year || !month || !day) return 'La fecha de nacimiento es requerida'
    ...
    const handleChange = (e) => {
        ...
        if (name === 'dob_day' || name === 'dob_month' || name === 'dob_year') {
            ...
            if (name === 'dob_day') d = value.padStart(2, '0')
            if (name === 'dob_month') m = value.padStart(2, '0')
            if (name === 'dob_year') y = value

            const newDate = `${y}-${m}-${d}`
            setFormData(prev => ({ ...prev, date_of_birth: newDate }))

            if (y && m && d && !newDate.includes('undefined')) {
                    ...
                    <div className="form-group">
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1.5fr', gap: '10px' }}>
                            <select
                                ...
                                disabled={loading}
                                style={{
                                    border: `2px solid ${dateError && formData.date_of_birth ? '#ff4444' : formData.date_of_birth.split('-')[2] && !dateError ? '#44cc44' : '#e5e7eb'}`,
                                }}
                            >
                            ...
                            <select
                                ...
                                disabled={loading}
                                style={{
                                    border: `2px solid ${dateError && formData.date_of_birth ? '#ff4444' : formData.date_of_birth.split('-')[1] && !dateError ? '#44cc44' : '#e5e7eb'}`,
                                }}
                            >
                            ...
                            <select
                                ...
                                disabled={loading}
                                style={{
                                    border: `2px solid ${dateError && formData.date_of_birth ? '#ff4444' : formData.date_of_birth.split('-')[0] && !dateError ? '#44cc44' : '#e5e7eb'}`,
                                }}
                            >
                        ...
                        {dateError && formData.date_of_birth && formData.date_of_birth.split('-').length === 3 && !formData.date_of_birth.includes('undefined') ? (
                            <small className="error-text">✗ {dateError}</small>
                        ) : formData.date_of_birth && !dateError && formData.date_of_birth.split('-')[0] ? (
